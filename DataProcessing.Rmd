...
http://rda.ucar.edu/datasets/ds502.0/ in monthly batches and was processed like this:
...
meses <- c('January','February','March','April','May','June','July',
           ...
           'January16','February16')

#Get extract daily ncdf files, accumulate precip sums by month
for (m in meses){
  setwd(paste0('/Users/matthewcooper/Creativitea/ENSO/Recent Trends/2015-2016/',m))
  all <- list.files()
  test <- nc_open(all[1])
  a <- ncvar_get(test, varid='cmorph_precip')
  testout <- apply(a, MARGIN=c(1,2), FUN=sum)
  s <- raster(t(testout), xmx=180, xmn=-180, ymx=60, ymn=-60)
  print(m)
  for (i in seq(2,length(all))){
    test <- nc_open(all[i])
    a <- ncvar_get(test, varid='cmorph_precip')
    testout <- apply(a, MARGIN=c(1,2), FUN=sum)
    r <- raster(t(testout), xmx=180, xmn=-180, ymx=60, ymn=-60)
    s <- sum(s,r,na.rm=T)
    print(i)
  }
  writeRaster(s, m, format='ascii')
}

#Get a raster from 1900-2014 dataset to use to compare extents
...
This is how the raster files were combined, and then test graphs were made using points form Uttar Pradesh and Los Angeles:
